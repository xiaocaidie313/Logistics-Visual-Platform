...
router.put('/order/update/:id', async (req:Request, res:Response) =>{
    try{
        ...
        const errorMessage = error instanceof Error ? error.message : '更改订单失败'; 
        sendResponse(res, 400, errorMessage, {});
...
router.get('/order/filter/pending', async (req:Request, res:Response) =>{
    try{
        const pendingOrders = await Goods.find({status:'pending'})
        sendResponse(res, 200, 'Success', pendingOrders)
    }catch(error:unknown){
        const errorMessage = error instanceof Error ? error.message :'获取待处理订单失败';
        sendResponse(res, 400, errorMessage, {})
    }
})
/// 1.2 paid状态
router.get('/order/filter/paid', async (req:Request, res:Response) =>{
    try{
        const paidOrders = await Goods.find({status:'paid'})
        sendResponse(res, 200, 'Success', paidOrders)
    }catch(error:unknown){
        const errorMessage = error instanceof Error ? error.message :'获取已支付订单失败';
        sendResponse(res, 400, errorMessage, {})
    }
})
/// 1.3 shipped状态
router.get('/order/filter/shipped', async (req:Request, res:Response) =>{
    try{
        const shippedOrders = await Goods.find({status:'shipped'})
        sendResponse(res, 200, 'Success', shippedOrders)
    }catch(error:unknown){
        const errorMessage = error instanceof Error ? error.message :'获取已发货订单失败';
        sendResponse(res, 400, errorMessage, {})
    }
})
/// 1.4 confirmed状态
router.get('/order/filter/confirmed', async (req:Request, res:Response) =>{
    try{
        const confirmedOrders = await Goods.find({status:'confirmed'})
        sendResponse(res, 200, 'Success', confirmedOrders)
    }catch(error:unknown){
        const errorMessage = error instanceof Error ? error.message :'获取已确认订单失败';
        sendResponse(res, 400, errorMessage, {})
    }
})
/// 1.5 delivered状态
router.get('/order/filter/delivered', async (req:Request, res:Response) =>{
    try{
        const deliveredOrders = await Goods.find({status:'delivered'})
        sendResponse(res, 200, 'Success', deliveredOrders)
    }catch(error:unknown){
        const errorMessage = error instanceof Error ? error.message :'获取已送达订单失败';
        sendResponse(res, 400, errorMessage, {})
    }
})
/// 1.6 cancelled状态
router.get('/order/filter/cancelled', async (req:Request, res:Response) =>{
    try{
        const cancelledOrders = await Goods.find({status:'cancelled'})
        sendResponse(res, 200, 'Success', cancelledOrders)
    }catch(error:unknown){
        const errorMessage = error instanceof Error ? error.message :'获取已取消订单失败';
        sendResponse(res, 400, errorMessage, {})
    }
})
...
router.get('/order/filter/refunded', async (req:Request, res:Response) =>{
    try{
        const refundedOrders = await Goods.find({status:'refunded'})
        sendResponse(res, 200, 'Success', refundedOrders)
    }catch(error:unknown){
        const errorMessage = error instanceof Error ? error.message :'获取已退款订单失败';
        sendResponse(res, 400, errorMessage, {})
    }
})  
// 2 订单排序
/// 2.1 按订单时间排序
router.get('/order/sort/ordertime/asc', async (req:Request, res:Response) =>{ //升序
    try{
        const sortedOrders = await Goods.find().sort({ordertime:1})
...
router.get('/order/sort/ordertime/des', async (req:Request, res:Response) =>{ //降序
    try{
        const sortedOrders = await Goods.find().sort({ordertime:-1})
        sendResponse(res, 200, 'Success', sortedOrders)
    }catch(error:unknown){
...
/// 2.2按照订单总的金额排序
router.get('/order/sort/totprice/asc', async (req:Request, res:Response)=>{ //升序
    try{
        const sortedOrders = await Goods.find().sort({totprice:1})
        sendResponse(res, 200, 'Success', sortedOrders)
    }catch(error:unknown){
